...
#define NCE_VERSION "0.1.0"
#define CTRL_KEY(k) ((k) & 0x1f)
#define TAB_STOP 8
#define QUIT_CONFIRM_COUNT 2
...
static int editor_row_cx_to_rx(erow *row, int cx) {
  int rx = 0;
  for (int j = 0; j < cx; j++) {
...
static void editor_row_insert_char(erow *row, int at, int c) {
...
static void editor_row_append_string(erow *row, char *s, size_t len) {
...
static char *editor_rows_to_string(int *buflen) {
  int total_len = 0;
  for (int j = 0; j < E.numrows; j++) total_len += E.row[j].size + 1;
  *buflen = total_len;
...
static void editor_save(void) {
  if (E.filename == NULL) {
    editor_set_status_message("No filename. Start with: ./nce path/to/file");
    return;
  }
  ...
  free(buf);
  editor_set_status_message("Cannot save: %s", strerror(errno));
}
...
static void editor_scroll(void) {
  E.rx = 0;
  if (E.cy < E.numrows) E.rx = editor_row_cx_to_rx(&E.row[E.cy], E.cx);
    ...
    if (filerow >= E.numrows) {
      if (E.numrows == 0 && y == E.screenrows / 3) {
        ...
        int welcomelen = snprintf(welcome, sizeof(welcome),
                                  "Native Code Editor -- version %s", NCE_VERSION);
...
static void editor_process_keypress(void) {
  static int quit_times = QUIT_CONFIRM_COUNT;
  ...
  switch (c) {
    ...
    case CTRL_KEY('q'):
      if (E.dirty && quit_times > 0) {
        editor_set_status_message("Unsaved changes. Press Ctrl-Q %d more time%s to quit.",
